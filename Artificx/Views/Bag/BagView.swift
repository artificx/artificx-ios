...
struct BagView: View {
    ...
    fileprivate func CartTotalInfo() -> some View {
        ...
        fileprivate func ScrollHeight() -> CGFloat {
            var heightOfScroll = (CGFloat(Dish.numInCart()) * 130) + 20
            if heightOfScroll > (UIScreen.main.bounds.height - 400) {
                    heightOfScroll = (UIScreen.main.bounds.height - 400)
            }
            return heightOfScroll
        }
                    ...
                    NavigationBarView()
                        ZStack(alignment: .top) {
                            VStack {
                                    ...
                                    if tabSelection == 1 {
                                        VStack(spacing: 10) {
                                            ForEach(Dish.inCart(), id: \.id) { dish in
                                                ItemCellTypeThree(dish: dish, updateOutside: self.$needsUpdate, flag: self.$detailsFlag)
                                            }
                                        }
                                        .padding(.horizontal, 15)
                                    }
                                ...
                                .disabled(false)
                                .frame(height: UIScreen.main.bounds.height - 410) // It should be dynamic...
